...
$(async function () {
	...
	const activityOverviewUrl = 'https://forum.mafiascum.net/app.php/activity_overview/';
	if (window.location.href.startsWith(activityOverviewUrl)) {
		...
		button.on('click', () => {
			...
			$('#page-body > form > table > tbody > tr').each((_index, element) => {
				const checkbox = $(element).find('td:nth-child(1) > input[type=checkbox]').first();
				if (checkbox.prop('checked')) {
					const usernameURL = $(element).find('td:nth-child(2) > a').first().attr('href');
					if (usernameURL) {
						const urlParams = new URLSearchParams(usernameURL.split('?')[1]);
