...
export const createChromeHandler = <TRouter extends AnyRouter>(opts: CreateChromeHandlerOptions<TRouter>) => {
	...
	browser.runtime.onConnect.addListener((port) => {
		...
		const onMessage = async (message: TRPCChromeRequest) => {
			...
			try {
				...
				// eslint-disable-next-line @typescript-eslint/no-explicit-any
				const procedureFn = segments.reduce((acc, segment) => acc[segment], caller as any) as AnyProcedure;
